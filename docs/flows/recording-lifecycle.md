...
4. RecordingManager.setupAndStartStream selects a display (primary or user-selected) and configures SCStream.
5. VideoStreamOutput receives screen frames (and audio if enabled) and writes them via AVAssetWriter.
...
- Display switching clears the buffer and restarts capture.
